...
def main(
    ...
    pipeline="LINC",
    run_type="calibrator",
    filtered_antennas="[CR]S*&",
    bad_antennas="[CR]S*&",
    Ateam_separation_file=None,
    solutions=None,
    clip_sources="",
    removed_bands="",
    min_unflagged=0.5,
    ...
    ## get bad antennas
    bad_antennas_list = list(
        filter(
            None,
            set(
                bad_antennas.replace(filtered_antennas, "")
                .replace("!", "")
                .replace("*", "")
                .replace("&", "")
                .split(";")
    ...
    if solutions:
        ...
        # `sol000` is a hard-coded solset name in facetselfcal
        solset = data.getSolset("sol000")
        antennas = sorted(solset.getAnt().keys())
        for antenna in antennas:
            json_output["metrics"][pipeline]["stations"].append(
                {"station": antenna, "removed": "no", "percentage_flagged": {}}
            )
        ...
        for solset_name in data.getSolsetNames():
            ...
            for soltab_name in soltabs:
                ...
                for _, weights, _, _ in soltab.getValuesIter(
                    returnAxes=axes, weight=True
                ):
                    weights = losoto.reorderAxes(weights, soltab.getAxesNames(), axes)
                for i, ant in enumerate(ants):
                    if ant not in antennas:
                        continue
                    flagged_solutions[soltab_name][ant] = 1.0 - float(
                        numpy.mean(weights[i])
                    )
                    if (
                        "percentage_flagged_solutions"
                        not in json_output["metrics"][pipeline]["stations"][
                            antennas.index(ant)
                        ].keys()
                    ):
                        json_output["metrics"][pipeline]["stations"][
                    ...
                    json_output["metrics"][pipeline]["stations"][antennas.index(ant)][
                        "percentage_flagged_solutions"
                    ][soltab_name] = (flagged_solutions[soltab_name][ant] * 100)
    ...
    if bad_antennas_list == []:
        ...
        print(
            "Additional antennas removed from the data: " + ", ".join(bad_antennas_list)
        )
    ...
    if Ateam_separation_file:
        ...
        if len(json_output["metrics"][pipeline]["close_sources"]) > 0:
            ...
            for i in range(len(json_output["metrics"][pipeline]["close_sources"])):
                Ateam_name = json_output["metrics"][pipeline]["close_sources"][i][
                    "source"
                ]
                if Ateam_name in clip_sources:
                    json_output["metrics"][pipeline]["close_sources"][i][
                        "mitigation"
                    ] = "clip"
                ...
                Ateam_list += (
                    json_output["metrics"][pipeline]["close_sources"][i]["source"] + ","
                )
            if clip_list != []:
            ...
            Ateam_list = (
                Ateam_list.rstrip(",")
                + "\n\tOf which were clipped: "
                + clip_list
            )
        else:
            Ateam_list = "NONE"
        print(
            "A-Team sources close to the phase reference center: " + Ateam_list + "\n"
        )

    removed_bands = ", ".join(
        list(
            filter(lambda a: a != "None", removed_bands.replace("out_", "").split(","))
        )
    )
    if removed_bands != "":
        print(
            "Removed bands due to an unsufficient fraction of unflagged data (< "
            + str(min_unflagged * 100)
            + "%): "
            + removed_bands
            + "\n"
        )
    ...
    for flagFile in flagFiles:
        ...
        if station_statistics == []:
            for antenna in antennas:
                json_output["metrics"][pipeline]["stations"].append(
                    {"station": antenna, "removed": "no"}
                )
        for antenna in antennas:
            index = [
                i
                for (i, item) in enumerate(station_statistics)
                if item["station"] == antenna
            ][0]
            if (
                "percentage_flagged"
                not in json_output["metrics"][pipeline]["stations"][index].keys()
            ):
                json_output["metrics"][pipeline]["stations"][index][
                    "percentage_flagged"
                ] = {}
            json_output["metrics"][pipeline]["stations"][index]["percentage_flagged"][
                state
            ] = (flagged_fraction_antenna[antenna] * 100)
        f.close()
    ...
    if solutions:
        ...
        soltab_len = (
            "{:^" + str(max([len(soltab_name) for soltab_name in soltab_names])) + "}"
        )
        soltab_names = " ".join(
            [soltab_len.format(soltab_name) for soltab_name in soltab_names]
        )
                ...
                for soltab_name in soltabs:
                    try:
                        values_to_print.append(
                            soltab_len.format(
                                "{:6.2f}".format(
                                    100 * flagged_solutions[soltab_name][antenna]
                                )
                                + "%"
                            )
                        )
        ...
        for state in states:
            try:
                values_to_print.append(
                    state_len.format(
                        "{:6.2f}".format(
                            station_statistics[index]["percentage_flagged"][state]
                        )
                        + "%"
                    )
                )
    ...
    for bad_antenna in bad_antennas_list:
        ...
        try:
            ...
            json_output["metrics"][pipeline]["stations"].append(
                {"station": bad_antenna, "removed": "yes"}
            )

    output_fname = (
        source + "_" + pipeline + "_" + run_type + "_summary.json"
    ).lstrip("_")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "flagFiles", nargs="+", help="List of flag files in JSON format"
    )
    parser.add_argument(
        "--pipeline", type=str, default="LINC", help="Name of the pipeline."
    )
    parser.add_argument(
        "--run_type", type=str, default="calibrator", help="Type of the pipeline"
    )
    parser.add_argument(
        "--filtered_antennas",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Location of the Ateam_separation JSON file",
    )
    parser.add_argument(
        "--solutions",
        type=str,
        default=None,
        help="Location of the solutions h5parm file",
    )
    parser.add_argument(
        "--clip_sources",
        type=str,
        default="",
        help="Comma-separated list of sources that were clipped",
    )
    parser.add_argument(
        "--removed_bands",
        type=str,
        default="",
        help="Comma-separated list of bands that were removed from the data",
    )
    parser.add_argument(
        "--min_unflagged",
        type=float,
        default=0.5,
        help="Minimum fraction of unflagged data per band",
    )
    ...
    main(
        flagFiles=args.flagFiles,
        pipeline=args.pipeline,
        run_type=args.run_type,
        filtered_antennas=args.filtered_antennas,
        bad_antennas=args.bad_antennas,
        Ateam_separation_file=args.Ateam_separation_file,
        solutions=args.solutions,
        clip_sources=args.clip_sources,
        removed_bands=args.removed_bands,
        min_unflagged=args.min_unflagged,
        refant=args.refant,
    )
